describe('Spread syntax에 대해 학습합니다.', function () {
  ...
  it('Rest Parameter는 함수의 전달인자를 배열로 다룰 수 있게 합니다.', function () {
    ...
    function getAllParamsByArgumentsObj() {
      return arguments; //('0': 'first', '1': 'second', '2':'third')
    }

    const restParams = getAllParamsByRestParameter(['first', 'second', 'third']);
    const argumentsObj = getAllParamsByArgumentsObj('first', 'second', 'third');

    expect(restParams).to.deep.equal(['first', 'second', 'third']);//...args
    ...
    const argsArr = Array.from(argumentsObj);  //객체 -> 배열로 변경 
    expect(Array.isArray(argsArr)).to.deep.equal(true);
    expect(argsArr).to.deep.equal(['first', 'second', 'third']);
    expect(argsArr === restParams).to.deep.equal(true);  // ->false : 유사배열 
  });
